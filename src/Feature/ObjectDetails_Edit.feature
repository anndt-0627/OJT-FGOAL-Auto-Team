...
Scenario Outline: User can edit percent of objective user has done until current time with valid data
    ...
    When User click on an <ObjectiveName> in My Objectives
    And User click on item Configure the objective calculation progress
    And User click on item Update progress manual
    And User input <ValidPercent> for progress value
    ...
    Examples:
    |OjectiveName	|ValidPercent	|Status		|
    |ChauTTB_01	 	|0				|AtRisk		|
    				|1				|AtRisk		|
    				|29				|AtRisk		|
    				|30				|BeHind		|
...
Scenario Outline: User can edit percent of objective user has done until current time when click on up-arrow of this field
    ...
    And User click on item Update progress manual
    And User click on button Update
    And User click on the up-arrow of field percent of objective user has done
